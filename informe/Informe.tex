...
un motor de búsqueda basado en el modelo vectorial. Se presentan la arquitectura y
...
\keywords{Recuperación de Información  \and Motor de Búsqueda \and Modelo Vectorial.}
\end{abstract}
...
%TODO Poner más cosas sobre los modelos?
...
Para la implementación del motor de búsqueda se escogió el modelo vectorial. Se escogió este
modelo por su relativa facilidad de implementación y comprensión teórica, además por su buen
desempeño sobre los demás modelos existentes. 
%TODO Seguir ampliando esto a medida que se le agreguen más cosas
...
tiene esta etapa es que en la implementación se realizan muchos cambios en el código fuente agregando
nuevas funcionalidades al procesamiento, además estas funcionalidades generalmente se pueden usar
...
\subsubsection{Comunicación con el motor de búsqueda}
...
que consuman de esta y que presenten la información de la manera que más le convenga.\\

Los principales endpoints de esta interfaz se encuentran:

\begin{itemize}
    \item GET \url{search/?query=query\&offset=offset} Devuelve una lista asociada a los
documentos relevanes de la búsqueda.
    \item GET \url{document/?document\_id=document\_id} Devuelve el contenido del documento asociado
al id dado.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Aplicar stemming a los tokens
    %TODO Mantener actualizado con el desarrollo del proyecto
\end{enumerate}
...
%TODO Implementar
Implementar algoritmo de retroalimentación
...
se presenta como un navegador web que permite realizar consultas. El usuario también puede
ver el contenido del documento recuperado si lo desea \ref{ui_fig}.
...
\begin{figure}
% \includegraphics[width=\textwidth]{user_interfae.jpg}
